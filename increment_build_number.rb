...
def find_and_increment_version_number_with_key(key, infoplist)
    ...
    if (old_value.index("a") != nil) # alpha
        ...
        growl_it "New bundle version: #{new_version.to_s}"
        #notify "Incrementing builder number to #{new_version.to_s}", :icon => :Xcode
        ...
        # this is hack to pass back the new version to the caller - using a module level var
        #
        #notify "Incrementing builder number to #{new_version.to_s}", :icon => :Xcode
...
if (config == "RELEASE" || config == "BETA")
    ...
    if (archive_action)
      # tc - don't increment itunes connect app version!
